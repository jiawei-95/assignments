...
//restructuring the avl tree to balanced tree
...
//return the union avl tree of T1 and T2
{
	...
	int rightHeight = 0;
	AVLTree *res = newAVLTree();
	// save all elements in T1 in tuple1 in ascending order
	BuildTupleArray(T1->root, tuple1, &index);
	index = 0;
	// save all elements in T2 in tuple2 in ascending order
	BuildTupleArray(T2->root, tuple2, &index);
	// save the union result in tuple3 and set l as the length of the tuple3
	l = UnionArray(tuple1, tuple2, tuple3, T1->size, T2->size);
	// build avl tree from the tuple3 and set the root of the avl tree
...
//return the intersection avl tree of T1 and T2
{
	...
	// save all elements in T2 in tuple2 in ascending order
	BuildTupleArray(T2->root, tuple2, &index);
	// save the intersection result in tuple3 and set l as the length of the tuple3
...
//find and delete node stroing (k,v) and return the result
{
	...
	else
	{
		...
		while(1)
		{
			// find the first unbalanced node from the deleted node to the root
			AVLTreeNode *unbalancedNode = FindUnbalancedNode(deletedNode);
			if(unbalancedNode != NULL)
			{
				// adjust the unbalaced subtree
				Restructuring(T, unbalancedNode);
			}
			else
			{
