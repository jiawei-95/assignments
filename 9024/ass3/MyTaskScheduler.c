...
int TaskScheduler(char *f1, char *f2, int m )
{
	...
	// This loop takes O(nlogn + nlogn + mlogm) time
	...
	while(1)
	{
		...
		// find all ready task from RHeap at schedule point and insert it into Dheap
		if(Min(RHeap) <= schedulePoint && Min(RHeap) >= 0)
		{
			...
			// get the min node from DHeap and execute it
			// check if it meets the deadline
			if(schedulePoint + planNode->Etime > planNode->Dline)
			{
				res = 0;
			}
			fprintf(fp2, "%d Core%d %d ", planNode->TaskName, coreName, schedulePoint);
			Insert(CoreHeap, schedulePoint + planNode->Etime, coreName, 0, 0, 0);		
		}
		else
		{
			//no ready task at schedule point in RHeap
			if(DHeap->size == 0)
			{
				// DHeap is empty
				// reset the schedule point of core heap
